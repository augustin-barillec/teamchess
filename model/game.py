...
class Game:
    def __init__(self, db, game_id):
        ...
        self.team_white_user_names = [
            ...
            for user_id in self.team_white_user_ids]

        self.team_black_user_names = [
            self.get_user_name(user_id)
            for user_id in self.team_black_user_ids]
    ...
    def get_user_name(self, user_id):
        return self.db.collection(
            constants.USERS).document(user_id).get().to_dict()['user_name']
    ...
    def can_play(self, user_id):
        ...
        return team_name == self.move_number_to_team_name(
            self.current_move_number) and self.has_started
    ...
    def get_display(self, user_id):
        ...
        display_quit_team_button = (
                not self.has_started and self.is_player(user_id))
        display_start_button = (
                not self.has_started and self.is_organizer(user_id) and
                len(self.team_white_user_ids) > 0 and
                len(self.team_black_user_ids) > 0)
