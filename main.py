...
def home():
    if 'user_id' not in flask.session:
        ...
        update_time, user_ref = db.collection(
            model.constants.USERS).add(user_dict)
    ...
    game_not_found = (
            ...
            not db.collection(
                model.constants.GAMES).document(game_id).get().exists)
...
def update_user_name():
    ...
    user_ref = db.collection(
        model.constants.USERS).document(user_id)
...
def create_game():
    ...
    update_time, game_ref = db.collection(
        model.constants.GAMES).add(game_dict)
